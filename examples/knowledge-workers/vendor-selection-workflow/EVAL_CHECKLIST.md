...
- Scoring values align with documented criteria.

## Completeness

- All vendors are evaluated across all required criteria.
- Tradeoffs and risks are explicitly documented.

## Constraint Compliance

- Only approved sources are used.
- Confidential details follow sharing boundaries.

## Quality

- Recommendation is clear, concise, and defensible.
- Evidence trail is easy for reviewers to audit.
...
- Plan, Analyze, Validate, and Approve phases each have explicit
  pass/fail decisions.
- Blocking failures stop progression until recovery evidence is
  recorded.
- Distribution is blocked when any blocking failure remains unresolved.

## Failure Pattern Checks

- Every failed checkpoint is mapped to a `FP-VENDOR-*` pattern ID in
  `SPEC.md`.
...
- No non-approved source was used for major claims.
- Distribution is blocked when unresolved blockers remain.

## Adoption Gate Checks

- Internal draft gate evidence is recorded before leadership pilot.
- Leadership pilot evidence is recorded before operational use.
- Gate decisions include named owner and go/no-go outcome.
- Rollback triggers were evaluated before each gate exit.
- Operational use is blocked when gate evidence is incomplete.
